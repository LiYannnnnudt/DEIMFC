...
%     q = qStart + qStride*qq;
%     for r = rNum 
betarow = [0.1:0.1:10]%[0.001,0.01,0.1,1,10,100,1000,10000:
for kk = 1:length(betarow) 
        gamma = betarow(kk);      
        for k = 1:times
            ...
            %--------- results ----------% 
            [~,label_out] = max(outU);   
            AC(kk,k)  = accuracy(Y,label_out');
            NMI(kk,k) = nmi(Y,label_out');
            [jaccard(kk,k), purity(kk,k)] = myClustMeasure(label_out',Y);
        end
end
